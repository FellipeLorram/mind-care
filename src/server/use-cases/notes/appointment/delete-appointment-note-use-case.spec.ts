...
import { DeleteAppointmentNoteUseCase } from './delete-appointment-note';

let sut: DeleteAppointmentNoteUseCase;
...
describe('Delete Appointment note Use Case', () => {
	beforeEach(async () => {
		...
		sut = new DeleteAppointmentNoteUseCase(
	...
	it('should be able to delete an appointment note', async () => {
		...
		const appointmentNoteExists = await appointmentNoteRepository.findById(appointmentNote.id);

		expect(appointmentNoteExists).toBeNull();
	});

	it('should not be able to delete an appointment note if user does not exist', async () => {
		const appointmentNote = await appointmentNoteRepository.create({
			content: 'any_content',
			appointment_id: appointmentId,
		});
	...
	it('should not be able to delete an appointment note if patient does not exist', async () => {
		const appointmentNote = await appointmentNoteRepository.create({
			content: 'any_content',
			appointment_id: appointmentId,
		});
	...
	it('should not be able to delete an appointment note if appointment does not exist', async () => {
		const appointmentNote = await appointmentNoteRepository.create({
			content: 'any_content',
			appointment_id: appointmentId,
		});
	...
	it('should not be able to delete an appointment note if appointment note does not exist', async () => {
		await expect(sut.execute({
